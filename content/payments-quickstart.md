...
> **Prefer a QR code?** Pass `"paymentMethod": "pix_qr"` on the same request and the response will carry a per-order dynamic PIX QR (`qrCodeString` + `qrCodeBase64`, valid for 15 minutes) instead of the static key. See [Create a payment](payments-create.md) for the full QR flow and rendering tips.
